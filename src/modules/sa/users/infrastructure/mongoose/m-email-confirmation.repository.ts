...
@Injectable()
export class MEmailConfirmationRepository implements IEmailConfirmationRepository {
    constructor(
    ...
    async getEmailConfirmationByCode(
        ...
        // @ts-ignore
        return new SqlEmailConfirmation(emailConfirmation._id, emailConfirmation.isConfirmed, emailConfirmation.confirmationCode, emailConfirmation.expirationDate)
    }

    async checkConfirmation(userId: string): Promise<boolean | null> {
        const result = await this.userModel.findOne({_id: new ObjectId(userId)})
            .select({
                'emailConfirmation.isConfirmed': 1
            })
