...
export class PairQuizGameController {
    constructor(
        ...
        @Inject(IQuizGameQueryRepository) protected gameQueryRepository: IQuizGameQueryRepository
    ) {
    }
    ...
    async getGameById(
        ...
        if(result.firstPlayerProgress.player.id !== userId || result.secondPlayerProgress?.player.id !== userId) {
            throw new ForbiddenException()
        }
