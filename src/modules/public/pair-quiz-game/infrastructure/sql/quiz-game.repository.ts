...
export class QuizGameRepository {
    ...
    async createGame(userId: string): Promise<ViewGame> {
        ...
        try {
            ...
            const game = await manager.getRepository(SqlGame).save(new SqlGame(questions)) // TODO save trable
            await manager.getRepository(SqlGameProgress).save(new SqlGameProgress(game.id, userId))
    ...
    private async getQuestions(): Promise<Questions[]> {
        const questionsCount = await this.getQuestionsCount()

        let questions = []
        for ( let i = questions.length; i < 5; questions.length) {
