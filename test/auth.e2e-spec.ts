...
describe('/auth', () => {
    ...
    describe('POST -> "/auth/registration-email-resending"', () => {
        ...
        it('Should`t resending confirmation code, if email not registrated', async () => {
            const response = await auth.resendingConfirmationCode({ email: faker.internet.email() })
            expect(response.status).toBe(HttpStatus.BAD_REQUEST)
            expect(response.body).toStrictEqual({ errorsMessages })
        })
    ...
    describe('POST -> "/auth/password-recovery"', () => {
        ...
        it('Shouldn`t send password recovery if input data has invalid email', async () => {
            const response = await auth.sendPasswordRecovery(preparedSecurity.email.notValid)
            expect(response.status).toBe(HttpStatus.BAD_REQUEST)
            expect(response.body).toEqual({ errorsMessages })
        })

        it('Shouldn`t return error even if current email is not' +
          'registered (for prevent user`s email detection)', async () => {
            const response = await auth.sendPasswordRecovery(preparedSecurity.email.notExist)
            expect(response.status).toBe(HttpStatus.NO_CONTENT)
        })
        ...
        it('Should update confirmation code', async () => {
            ...
            const response = await auth.sendPasswordRecovery(preparedSecurity.email.valid)
            expect(response.status).toBe(HttpStatus.NO_CONTENT)
    ...
    describe('POST -> "/auth/new-password"', () => {
        it('Shouldn`t confirm password recovery if incorrect input dat', async () => {
            ...
            const response = await auth.newPassword(preparedPassword.long.password, randomCode)
            expect(response.status).toBe(HttpStatus.BAD_REQUEST)
            expect(response.body).toEqual({ errorsMessages })
        })
        ...
        it('Shouldn`t confirm password recovery if recovery code expired', async () => {
            ...
            const response = await auth.newPassword(preparedPassword.long.password, expiredCode)
            expect(response.status).toBe(HttpStatus.BAD_REQUEST)
            expect(response.body).toEqual({ errorsMessages })
        })
    ...
    describe('POST -> "/auth/refresh-token"', () => {
        it('Shouldn`t generate new pair token if the JWT refreshToken' +
          'inside cookie is missing', async () => {
        ...
        it('Shouldn`t generate new pair token if the JWT' +
          'inside cookie is expired', async () => {
            const { refreshToken } = expect.getState()
